...
} from '../features/dynamic-intake/types';
import { DEFAULT_PROMPTS, DEFAULT_INITIAL_STEP } from '../features/dynamic-intake/constants';
  ...
  devtools(
    persist(
      ...
      }),
      {
        ...
        partialize: (state) => ({
            // Don't persist large files or temporary UI states
            theme: state.theme,
            customConnection: state.customConnection,
            systemPrompts: state.systemPrompts,
            selectedModelId: state.selectedModelId,
            dynamicContext: state.dynamicContext // Persist context to keep chat state
        })
      }
    )
  )
