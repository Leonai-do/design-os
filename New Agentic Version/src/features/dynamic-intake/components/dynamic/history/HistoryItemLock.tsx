    ...
    <div className="w-full ml-14 bg-amber-50 dark:bg-amber-900/10 p-6 rounded-2xl border-2 border-amber-400 dark:border-amber-600 shadow-xl animate-fade-in relative z-10">
        <div className="mb-4">
            <h4 className="font-bold text-amber-800 dark:text-amber-400 flex items-center gap-2">
                <Lock className="w-4 h-4" /> Select Fields to Keep (Lock)
            </h4>
        ...
        <div className="space-y-3 bg-white dark:bg-zinc-800 p-4 rounded-xl border border-amber-200 dark:border-amber-800/50">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors
                                ${isLocked ? 'bg-amber-500 border-amber-500 text-white' : 'border-zinc-300 dark:border-zinc-600'}`}>
                                {isLocked && <Lock className="w-3 h-3" />}
                            </div>
                            <div>
                                <span className={`text-sm font-bold block ${isLocked ? 'text-amber-900 dark:text-amber-100' : 'text-zinc-700 dark:text-zinc-300'}`}>{field.label}</span>
                                <span className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-[200px] block">
                        ...
                        {isLocked ? (
                            ...
                            <span className="text-xs text-zinc-400">Regenerate</span>
                        )}
                    </div>
                );
            })}
        ...
        <div className="mt-6 flex justify-end gap-3">
            <button onClick={onCancel} className="px-4 py-2 text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 text-sm font-medium">
                Cancel
            </button>
            <button onClick={() => onConfirm(Array.from(lockedFieldIds))} className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-bold text-sm shadow transition-colors flex items-center gap-2">
                <RefreshCw className="w-4 h-4" /> Regenerate Step
            </button>
        </div>
    </div>
  );
