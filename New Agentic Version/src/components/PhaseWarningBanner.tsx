...
export function PhaseWarningBanner() {
  ...
  const hasDesignSystem = !!(data.designSystem?.colors || data.designSystem?.typography)
  const hasShell = !!data.shell?.spec
  const hasDesign = hasDesignSystem || hasShell

  const productName = data.overview?.name || 'default-product'
